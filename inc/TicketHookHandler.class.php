...
class PluginGSpecialistATicketHookHandler extends CommonDBTM implements IPluginGSpecialistAHookItemHandler {

    protected $DB;
    protected $gsAssignmentsEntity;

    public function __construct() {
        global $DB;

        $this->DB = $DB;
        $this->rrAssignmentsEntity = new PluginGSpecialistAGSAssignmentsEntity();
    }
    ...
    public function itemAdded(CommonDBTM $item) {
        ...
        PluginGSpecialistALogger::addWarning(__METHOD__ . " - TicketId: " . $this->getTicketId($item));
        PluginGSpecialistALogger::addWarning(__METHOD__ . " - CategoryId: " . $this->getTicketCategory($item));
...
EOT;
        ...
        PluginGSpecialistALogger::addWarning(__METHOD__ . ' - result array: ', $resultArray);
        return $resultArray;
    }
    ...
    protected function assignTicket(CommonDBTM $item) {
        ...
        if (($lastAssignmentIndex = $this->getLastAssignmentIndex($item)) === false) {
            PluginGSpecialistALogger::addWarning(__FUNCTION__ . ' - nothing to to (category is disabled or not configured; getLastAssignmentIndex: ' . $lastAssignmentIndex);
            return;
        }
...
EOT;
        PluginGSpecialistALogger::addWarning(__FUNCTION__ . ' - sqlDelete_glpi_tickets_users: ' . $sqlDelete_glpi_tickets_users);
...
EOT;
        ...
        PluginGSpecialistALogger::addWarning(__FUNCTION__ . ' - sqlDelete_glpi_groups_tickets: ' . $sqlDelete_glpi_groups_tickets);
...
EOT;
        PluginGSpecialistALogger::addWarning(__FUNCTION__ . ' - sqlInsert_glpi_tickets_users: ' . $sqlInsert_glpi_tickets_users);
...
EOT;
            PluginGSpecialistALogger::addWarning(__FUNCTION__ . ' - sqlInsert_glpi_tickets_groups: ' . $sqlInsert_glpi_tickets_groups);
            $this->DB->queryOrDie($sqlInsert_glpi_tickets_groups, $this->DB->error());
        }
    }

    public function itemPurged(CommonDBTM $item) {
        PluginGSpecialistALogger::addWarning(__FUNCTION__ . ' - nothing to do');
    }
