 ...
 * -------------------------------------------------------------------------
 * rr gsa 
 * $rrAssignmentTable $gsAssignmentTable
 * $rrOptionsTable $gsOptionsTable
 * PluginGSpecialistARRAssignmentsEntity PluginGSpecialistAGSAssignmentsEntity
 */
...
class PluginGSpecialistAGSAssignmentsEntity extends CommonDBTM {

    protected $DB;
    protected $gsAssignmentTable;
    protected $gsOptionsTable;

    public function __construct() {
        global $DB;

        $this->DB = $DB;
        $this->gsAssignmentTable = PluginGSpecialistAConfig::getgsAssignmentTable();
        $this->gsOptionsTable = PluginGSpecialistAConfig::getgsOptionsTable();
    }
    ...
    public function cleanUp() {
         ...
         * drop settings
         */
        if ($this->DB->tableExists($this->gsAssignmentTable)) {
            $sqlDropAssign = <<< EOT
            DROP TABLE {$this->gsAssignmentTable}
EOT;
            PluginGSpecialistALogger::addWarning(__FUNCTION__ . ' - sqlDrop: ' . $sqlDropAssign);
            $this->DB->queryOrDie($sqlDropAssign, $this->DB->error());
        } else {
            PluginGSpecialistALogger::addWarning(__FUNCTION__ . " - table not dropped because it does not exist: " . $this->gsAssignmentTable);
        }

        /**
         * drop options
         */
        if ($this->DB->tableExists($this->gsOptionsTable)) {
            $sqlDropOptions = <<< EOT
            DROP TABLE {$this->gsOptionsTable}
EOT;
            PluginGSpecialistALogger::addWarning(__FUNCTION__ . ' - sqlDrop: ' . $sqlDropOptions);
            $this->DB->queryOrDie($sqlDropOptions, $this->DB->error());
        } else {
            PluginGSpecialistALogger::addWarning(__FUNCTION__ . " - table not dropped because it does not exist: " . $this->gsOptionsTable);
        }
    }
    ...
    protected function createTable() {
        ...
        if (!$this->DB->tableExists($this->gsAssignmentTable)) {
            $sqlCreateAssign = <<< EOT
                    CREATE TABLE IF NOT EXISTS {$this->gsAssignmentTable} (
...
EOT;
            PluginGSpecialistALogger::addWarning(__FUNCTION__ . ' - sqlCreate: ' . $sqlCreateAssign);
        ...
        if (!$this->DB->tableExists($this->gsOptionsTable)) {
            $sqlCreateOption = <<< EOT
                    CREATE TABLE IF NOT EXISTS {$this->gsOptionsTable} (
                        id INT(11) NOT NULL auto_increment,
                        auto_assign_group INT(1) DEFAULT 1,
                        PRIMARY KEY (id)
                    ) DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci
EOT;
            PluginGSpecialistALogger::addWarning(__FUNCTION__ . ' - sqlCreate: ' . $sqlCreateOption);
    ...
    protected function truncateTable() {
        ...
        if ($this->DB->tableExists($this->gsAssignmentTable)) {
            $sqlTruncAssign = <<< EOT
                TRUNCATE TABLE {$this->gsAssignmentTable}
EOT;
            PluginGSpecialistALogger::addWarning(__FUNCTION__ . ' - sqlTrunc: ' . $sqlTruncAssign);
        ...
        if ($this->DB->tableExists($this->gsOptionsTable)) {
            $sqlTruncOptions = <<< EOT
                TRUNCATE TABLE {$this->gsOptionsTable}
EOT;
            PluginGSpecialistALogger::addWarning(__FUNCTION__ . ' - sqlTrunc: ' . $sqlTruncOptions);
...
EOT;
        $sqlCategory = <<< EOT
                SELECT id FROM glpi_itilcategories
EOT;
        PluginGSpecialistALogger::addWarning(__FUNCTION__ . ' - sqlCategory: ' . $sqlCategory);
    ...
    public function insertItilCategory($itilCategory) {
         ...
         */
        $sqlInsert = <<< EOT
                INSERT INTO {$this->gsAssignmentTable} (itilcategories_id) VALUES ({$itilCategory})
EOT;
        PluginGSpecialistALogger::addWarning(__FUNCTION__ . ' - sqlInsert: ' . $sqlInsert);
    ...
    public function insertOptions() {
         ...
         */
        $sqlInsert = <<< EOT
                INSERT INTO {$this->gsOptionsTable} (auto_assign_group) VALUES (1)
EOT;
        PluginGSpecialistALogger::addWarning(__FUNCTION__ . ' - sqlInsert: ' . $sqlInsert);
        $this->DB->queryOrDie($sqlInsert, $this->DB->error());
    }
    ...
    public function getOptionAutoAssignGroup() {
        $sql = <<< EOT
                SELECT auto_assign_group FROM {$this->gsOptionsTable} LIMIT 1
EOT;
        PluginGSpecialistALogger::addWarning(__FUNCTION__ . ' - sql: ' . $sql);
...
EOT;
    ...
    public function updateAutoAssignGroup($autoAssignGroup) {
        $sqlUpdate = <<< EOT
                UPDATE {$this->gsOptionsTable}
                SET auto_assign_group = {$autoAssignGroup}
                WHERE id = 1
EOT;
        PluginGSpecialistALogger::addWarning(__FUNCTION__ . ' - sqlUpdate: ' . $sqlUpdate);
    ...
    public function deleteItilCategory($itilCategory) {
         ...
         */
        $sqlDelete = <<< EOT
                DELETE FROM {$this->gsAssignmentTable} WHERE itilcategories_id = {$itilCategory}
EOT;
        PluginGSpecialistALogger::addWarning(__FUNCTION__ . ' - sqlDelete: ' . $sqlDelete);
        $this->DB->queryOrDie($sqlDelete, $this->DB->error());
    }

    public function updateLastAssignmentIndex($itilcategoriesId, $index) {
        $sqlUpdate = <<< EOT
                UPDATE {$this->gsAssignmentTable}
...
EOT;
    ...
    public function getLastAssignmentIndex($itilcategoriesId) {
        $sql = <<< EOT
                SELECT last_assignment_index FROM {$this->gsAssignmentTable} 
                WHERE itilcategories_id = {$itilcategoriesId} AND is_active = 1
EOT;
        PluginGSpecialistALogger::addWarning(__FUNCTION__ . ' - sql: ' . $sql);
        $resultCollection = $this->DB->queryOrDie($sql, $this->DB->error());
        $resultArray = iterator_to_array($resultCollection);
        PluginGSpecialistALogger::addWarning(__FUNCTION__ . ' - $resultArray: ' . print_r($resultArray, true));
        if (count($resultArray) === 0 || count($resultArray) > 1) {
            ...
            PluginGSpecialistALogger::addWarning(__FUNCTION__ . ' - category has entry');
    ...
    public function getAll() {
        $sql_0 = <<< EOT
                SELECT 
                    ...
                    a.is_active
                FROM
                    glpi_plugin_GSPECIALISTA_rr_assignments a
                        JOIN
