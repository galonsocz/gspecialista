 ...
 * -------------------------------------------------------------------------
 */
class PluginGSpecialistARequest {
    ...
    public static function getCurrentProfileSettings() {
        ...
        $pluginCode = PluginGSpecialistAConfig::$PLUGIN_GSPECIALISTA_CODE;
...
EOT;
        ...
        PluginGSpecialistALogger::addWarning(__METHOD__ . " - profile_id: $profileId results: " . var_export($settingsArray, true));
    ...
    public static function initJavaScriptBlock() {
        ...
        $plugin = new Plugin();
        if (!$plugin->isActivated(PluginGSpecialistAConfig::$PLUGIN_GSPECIALISTA_CODE)) {
            exit;
        }
    }
